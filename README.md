...
- **ARM-NEON SIMD reactivated** for libjpeg-turbo's MJPEG decode path (every frame from the camera, both live preview and capture, is JPEG-decoded natively — see "How the app works"). Contrary to the original assumption that this needs a separate assembler (true only for the x86 NASM path), the ARM `.S` files are plain GNU-assembler syntax that CMake's standard `enable_language(ASM)` builds directly via Clang's integrated assembler — no extra toolchain setup needed. x86_64 stays on the portable `jsimd_none.c` fallback (no real device targets it, only the emulator, and it would need actual NASM). **Measured live** (Xiaomi 11T Pro 5G, 4 repeated dual-camera capture rounds each): full-resolution parallel dual-capture dropped from **3295ms → 2950ms average, ~10% faster** (see `libuvccamera/src/main/jni/CMakeLists.txt`)
